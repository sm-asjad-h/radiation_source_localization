import numpy as np
import pandas as pd
def generate_dataset_fully_constrained(num_samples=30000, area_size=18, del_cons=1.2, back_rad=0.0,SI_min=400.0, SI_max=8000.0, dist_min=0.5,min_angle=25.0, max_angle=130.0, min_area=10.0):
    data = []
    for sample_idx in range(num_samples):
        ...
        while not geom_valid:
            ...
            valid_angles = (min_angle <= angle_A <= max_angle) and (min_angle <= angle_B <= max_angle) and (min_angle <= angle_C <= max_angle)

            if valid_angles and (triangle_area >= min_area):
                geom_valid = True
